...
Preview repair: user reported a local page-load error. Vite logged an outdated React JSX runtime export mismatch during late Base UI dependency optimization. Explicitly prebundling the context controls in vite.config.ts avoids that late discovery pass. Production build had passed; browser interaction QA remains unperformed.

Latest visual update (September 10): Life context is now an inline radio-card section on the home screen above the reflection input, with automatic saving and a plain-language explanation. The header shortcut remains on other screens only. User-facing wording is Life context, not Practice context. Legacy teal/green/emerald/cyan/lime utilities now resolve to blue tokens, and game accent materials are being aligned to blue. Grass/environment scenery is not a UI accent and is preserved. Context content and personal reflection behavior are unchanged.
